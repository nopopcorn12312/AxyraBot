...
// EnsureSevenTVColumns runs idempotent ALTER TABLE statements to add the
// seventv_token column to broadcaster_settings and the role column to
// default_command_settings if they do not already exist.
func EnsureSevenTVColumns() error {
	if db == nil {
		return nil
	}
	_, err := db.Exec(`
ALTER TABLE broadcaster_settings ADD COLUMN IF NOT EXISTS seventv_token TEXT NOT NULL DEFAULT '';
ALTER TABLE default_command_settings ADD COLUMN IF NOT EXISTS role TEXT NOT NULL DEFAULT 'moderator';
`)
	return err
}

// Get7TVToken returns the stored 7TV API token for a broadcaster.
func Get7TVToken(broadcasterLogin string) (string, error) {
	if db == nil {
		return "", nil
	}
	broadcasterLogin = strings.ToLower(strings.TrimSpace(broadcasterLogin))
	var token string
	row := db.QueryRow(`SELECT seventv_token FROM broadcaster_settings WHERE broadcaster_login=$1`, broadcasterLogin)
	if err := row.Scan(&token); err != nil {
		if err == sql.ErrNoRows {
			return "", nil
		}
...
// Set7TVToken upserts the 7TV API token for a broadcaster.
func Set7TVToken(broadcasterLogin, token string) error {
	if db == nil {
		return nil
	}
	broadcasterLogin = strings.ToLower(strings.TrimSpace(broadcasterLogin))
	_, err := db.Exec(`
INSERT INTO broadcaster_settings (broadcaster_login, seventv_token)
VALUES ($1, $2)
ON CONFLICT (broadcaster_login) DO UPDATE
SET seventv_token = EXCLUDED.seventv_token;
`, broadcasterLogin, token)
	return err
}

// GetDefaultCommandRole returns the role required to use a built-in command.
// If no row exists the default is "moderator".
func GetDefaultCommandRole(broadcasterLogin, commandName string) (string, error) {
	if db == nil {
	...
	commandName = strings.ToLower(commandName)
	var role string
	row := db.QueryRow(`SELECT role FROM default_command_settings WHERE broadcaster_login=$1 AND command_name=$2`, broadcasterLogin, commandName)
...
// SetDefaultCommandRole upserts the role for a built-in command.
func SetDefaultCommandRole(broadcasterLogin, commandName, role string) error {
	if db == nil {
		return nil
	}
	broadcasterLogin = strings.ToLower(broadcasterLogin)
	commandName = strings.ToLower(commandName)
	_, err := db.Exec(`
INSERT INTO default_command_settings (broadcaster_login, command_name, enabled, role)
VALUES ($1, $2, true, $3)
ON CONFLICT (broadcaster_login, command_name) DO UPDATE
SET role = EXCLUDED.role;
`, broadcasterLogin, commandName, role)
	return err
}
